...
# ===================================================================
# BOLT Platform: DeepAligned 算法启动脚本 (v2, 含耗时日志)
# ===================================================================
...
# --- 新增：日志设置 ---
mkdir -p time_log
LOG_FILE="time_log/deepaligned_time.log"
echo "================== DeepAligned Run @ $(date '+%F %T') ==================" | tee -a "$LOG_FILE"
total_start=$(date +%s)
...
do
    ...
    do
        echo "========================================================================" | tee -a "$LOG_FILE"
        echo "Running DeepAligned with: dataset=${dataset}, kcr=${known_cls_ratio}, labeled_ratio=${labeled_ratio}, fold=${fold_idx}, seed=${seed}" | tee -a "$LOG_FILE"
        echo "========================================================================" | tee -a "$LOG_FILE"
        ...
        # --- 3. 执行区 (直接调用) ---
        python code/gcd/baselines/DeepAligned-Clustering/run.py \
            --config $CONFIG_FILE \
            --dataset $dataset \
        ...
        echo "Finished DeepAligned task: dataset=${dataset}, kcr=${known_cls_ratio}, labeled_ratio=${labeled_ratio}, fold=${fold_idx}, seed=${seed}" | tee -a "$LOG_FILE"
...
echo "All DeepAligned experiments have been completed." | tee -a "$LOG_FILE"
