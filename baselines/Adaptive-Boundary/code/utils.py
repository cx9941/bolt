...
class Split:
    ...
    def get_X_y(self, lst, limit_num_sents, set_type: str):
        ...
        # If embedding function exists, apply it in batches
        if self.embed_f is not None:
            batch_size = 16
            X_embedded = []
            for idx in range((len(X) - 1) // batch_size + 1):
                X_embedded_batch = self.embed_f(X[idx*batch_size:(idx+1)*batch_size])
                X_embedded.append(X_embedded_batch)
            X = tf.concat(X_embedded, axis=0)
