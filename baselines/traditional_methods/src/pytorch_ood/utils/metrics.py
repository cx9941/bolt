...
class OODMetrics(object):
    ...
    def _compute(self, labels: Tensor, scores: Tensor) -> Dict[str, Tensor]:
        ...
        return {
            ...
            "FPR95TPR": fpr.cpu(),
            "ACC": sup_metrics['accuracy'],
            "F1-Score": sup_metrics['macro avg']['f1-score'],
        }
...
def custom_metrics(preds, golds, scores=None, thred=0.5):
    if scores is None:
        ...
        results['K-f1'] = np.array([v['f1-score'] for i,v in metrics.items() if int(i) >= 0]).mean()
        results['N-f1'] = np.array([v['f1-score'] for i,v in metrics.items() if int(i) == -1]).mean()
    return results
