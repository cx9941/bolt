...
class ALManager:
    ...
    def al_finetune(self, args):
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            results = self.test(test_dataloader=self.test_dataloader, model=self.model, num_labels=self.num_labels, args=args)
            ...
            if results['ACC'] + results['ARI'] + results['NMI'] > \
                ...
                # 直接用新的、更好的 results 字典更新 best_metrics
    ...
    def llm_labeling(self, args, epoch, model):
        ...
        if not os.path.exists(os.path.join(args.result_dir, f'llm_annotated_output_{args.seed}_{args.known_cls_ratio}_{epoch}.json')):
            ...
            for i, cluster_ind in tqdm(enumerate(np.unique(y_pred)), desc="LLM Labeling Iter", total=len(np.unique(y_pred)), leave=False, dynamic_ncols=True):
                ...
                num_nearst_clusters = int(self.num_labels * args.comparison_cluster_ratio)
                ...
                if args.dataset == 'banking':
                    ...
                    comparison_prompting = PROMPT_MCID.format(utterance_set, max_uncertain_utterance_text)
                elif args.dataset == 'hwu':
                    comparison_prompting = PROMPT_HWU.format(utterance_set, max_uncertain_utterance_text)
